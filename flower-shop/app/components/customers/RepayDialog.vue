      ...
      >
        <a-form-item label="还款金额" name="amount">
          <a-input-number
            ...
            autofocus
          />
          <div v-if="customer.balance < 0" class="mt-1 text-xs text-gray-500">
            提示：可一键填入欠款 <a class="text-pink-600" @click="form.amount = Math.abs(customer.balance)">¥{{ Math.abs(customer.balance).toFixed(2) }}</a>
          </div>
        </a-form-item>
...
const { repayCustomer } = useCustomers()
  ...
  amount: [
    { required: true, message: '请输入还款金额', trigger: 'blur' },
    {
      validator: (_rule: any, value: number) => {
        if (value === undefined || value === null || isNaN(value)) return Promise.reject('请输入还款金额')
...
const onSubmit = async () => {
  ...
  try {
    const result = await repayCustomer(props.customer.id, {
      amount: form.amount!,
      paymentMethod: form.paymentMethod,
      notes: form.notes || undefined,
    })
    message.success(`已收款 ¥${form.amount!.toFixed(2)}`)
    emit('success', result)
