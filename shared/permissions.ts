...
export const permissions: Record<string, Role[]> = {
  ...
  'preorder.write': all, 'stock.write': managers, 'purchase.write': managers,
  'shift.write': all, 'notification.write': all, 'admin': ['admin'],
}
...
export function routeAction(method: string, path: string): string | null {
  ...
  if (/^\/api\/(suppliers|purchases)(\/[^/]+){0,3}$/.test(path)) return 'purchase.write'
  if (/^\/api\/shifts\/\d+\/reopen$/.test(path)) return 'admin'
  if (/^\/api\/shifts(\/[^/]+){0,2}$/.test(path)) return 'shift.write'
  if (/^\/api\/notifications(\/\d+\/read|\/mark-all-read)$/.test(path)) return 'notification.write'
  if (/^\/api\/(settings|users|imports|reconciliation|loyalty-rules|notifications\/regenerate)(\/[^/]+){0,2}$/.test(path)) return 'admin'
  return null
}
