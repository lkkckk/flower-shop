...
export default defineEventHandler(async (event) => {
  ...
  if (body.version === undefined || body.version === null) {
    throw createError({ statusCode: 400, message: '请提供版本号进行并发校验' })
  }
  ...
  const action = 'preorder_registration.update'
  const opId = `${actor}:${action}:${key}`
  const requestHash = createHash('sha256').update(JSON.stringify({ path: `/api/preorder-registrations/${id}`, body })).digest('hex')
