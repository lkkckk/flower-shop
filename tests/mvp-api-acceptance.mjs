...
try {
  ...
  await call('/api/loyalty-rules', 'staff', 'PUT', { earnPerYuan: 999 }, 404)
  ...
  const cost = await p.orderCostAllocation.aggregate({ where: { orderId: recipeSale.order.id }, _sum: { totalCost: true } })
  ...
  await call('/api/reconciliation','cashier','GET',undefined,404)
  ...
  await call('/api/imports/preview', 'admin', 'POST', {}, 404)
  console.log(JSON.stringify({ status: 'passed', checks: ['RBAC', 'member pricing', 'concurrent idempotency', 'partial refund', 'points', 'cashier image upload', 'freeform registration', 'recipe allocation', 'report and statement', 'retired endpoints'], customerId: c.id, orderId: sale.order.id, registrationId: pre.id, username: `mvp_admin_${suffix}` }))
